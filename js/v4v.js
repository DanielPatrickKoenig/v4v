...
var v4v = (function() {
    function V4V(){
        ...
        this.resizorWithRotation = function(_stage, _element, _handler,_offset,_enableDelete){
            return new ResizorWithRotation(_stage, _element,_handler,_offset,_enableDelete);
        }

        function ResizorWithRotation(_stage, _element,_handler,_offset,_enableDelete){
        ...
        this.resizableStage = function(_stage){
            return new ResizeableStage(_stage);
        }

        function ResizeableStage(_stage){
            ...
            function onMarkerSelected(cm){
                ...
                resizor = new ResizorWithRotation(_stage, cm ,onResize,resizorOffset,true);
                //currentMarker = e.currentTarget;
                //document.getElementById("id_field_"+svgID).value = cm.getAttribute("markerid");
            }

            function onMarkerDown(){
                if(resizor != undefined){
                    resizor.remove();
                    resizor = undefined;
                    currentMarker = undefined;
        ...
        function onMove(stage, container, element, e, dragProperties, handler) {
            if (container.isDragging) {
                ...
                if (handler != null && handler != undefined) {
                    handler({ x: Number(container.selecitonOffset.x) + Number(e.pageX), y: Number(container.selecitonOffset.y) + Number(e.pageY) });
                }
            }
        }
